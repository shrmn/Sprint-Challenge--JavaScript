...
const newunilist = graduates.map((university) => {
...
const matches = graduates.reduce((total, unis) => {
  return total += unis.university.includes("Uni");
}, 0);
...
const lowerCase = zooAnimals.map(result => {
  return result.animal_name.toLowerCase();
})
